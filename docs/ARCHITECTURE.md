...
If leaderboards or cloud save are added, the app talks to the backend only through a documented REST API (add the schema here once it exists) — never by reaching into backend internals. See `infra/docker/README.md`, `infra/k8s/README.md` for the optional deployment scaffolding.
