...
Applies to `app/src/main/java/com/example/gametemplate/ui/` — menus, HUD overlays, settings, pause screens. Does **not** apply to the `SurfaceView` game surface itself (see [`game_loop_performance.md`](game_loop_performance.md)).
...
- Match Material 3 theming already set up in `app/src/main/res/values/themes.xml`; don't introduce a second design system for one screen.
